...
class TagManagerMixin:
    ...
    def setup_tag_ui(self, parent_layout):
        ...
        self.add_tag_btn = QPushButton(_("添加标签"))
        ...
        self.manage_specific_tag_btn = QPushButton(_("管理标签"))
    ...
    def filter_tags(self, keyword=""):
        ...
        if keyword:
            self.tag_search_input.setPlaceholderText(f"{_('找到')} {len(filtered_tags)} {_('个标签')}")
        else:
    ...
    def add_new_tag(self):
        """添加新标签"""
        tag_name, ok = QInputDialog.getText(self, _("添加新标签"), _("请输入标签名称:"))
        if ok and tag_name.strip():
            controller = self.get_controller()
            result = controller.add_tag(tag_name.strip())
            if result:
                QMessageBox.information(self, _("成功"), f"{_('标签')} '{tag_name}' {_('添加成功！')}")
                self.load_tags()
            else:
                QMessageBox.warning(self, _("错误"), f"{_('标签')} '{tag_name}' {_('已存在或添加失败！')}")
    ...
    def manage_specific_tag(self):
        ...
        if not all_tags:
            QMessageBox.information(self, _("提示"), _("当前没有标签。"))
            return
        
        tag_names = [tag['name'] for tag in all_tags]
        selected_tag, ok = QInputDialog.getItem(
            self, _("选择标签"), _("选择要管理的标签:"), tag_names, 0, False
        )
        ...
        if usage_count > 0:
            QMessageBox.information(
                self, _("标签正在被使用"),
                f"{_('标签')} '{selected_tag}' {_('正在被')} {usage_count} {_('篇日记使用，无法删除。')}"
            )
        else:
            reply = QMessageBox.question(
                self, _("确认删除未使用标签"),
                f"{_('标签')} '{selected_tag}' {_('未被任何日记使用，是否删除？')}",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
            )
            ...
            if reply == QMessageBox.StandardButton.Yes:
                try:
                    ...
                    if success:
                        QMessageBox.information(self, _("成功"), f"{_('标签')} '{selected_tag}' {_('已删除。')}")
                        self.load_tags()
                    else:
                        QMessageBox.warning(self, _("失败"), f"{_('删除标签')} '{selected_tag}' {_('失败。')}")
                except Exception as e:
                    QMessageBox.critical(self, _("错误"), f"{_('删除标签时发生错误:')} {str(e)}")
